    ...
    <div class="flex flex-1 overflow-hidden relative">
      ...
      <div class="discover-content">
        ...
        <div class="discover-header shrink-0 pt-6 px-6 pb-2 z-10 bg-[#121212]">
          ...
          <p class="text-gray-400 mb-6 text-sm">Explorez des playlists YouTube et lancez-les instantanément.</p>
        ...
        >
          ...
          <div class="mt-6 mb-8" v-if="!searchQuery && results.length === 0 && !isLoading">
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
              ...
              >
                ...
                <div class="absolute inset-0 bg-gradient-to-r opacity-0 group-hover:opacity-10 transition-opacity" :class="suggestion.color"></div>
          ...
          <!-- État Chargement -->
          <div v-if="isLoading" class="flex flex-col items-center justify-center text-gray-400 py-12">
          ...
          <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            ...
            >
              <!-- Image -->
              <div class="relative w-full aspect-video mb-3 overflow-hidden rounded-md bg-gray-800 shrink-0">
                <img :src="playlist.thumbnail" class="w-full h-full object-cover" />
                
                <!-- Overlay Play -->
                <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity">
                  <button class="w-12 h-12 bg-green-500 rounded-full flex items-center justify-center text-black shadow-lg transform scale-90 group-hover:scale-100 transition-transform">
                    <i class="pi pi-play text-lg ml-1"></i>
                  </button>
                </div>

                <!-- Badge Count -->
                <div class="absolute bottom-2 right-2 bg-black/80 px-1.5 py-0.5 rounded text-xs font-bold text-white flex items-center gap-1">
              ...
              <div class="flex-1 flex flex-col min-w-0">
                <h3 class="font-bold text-white truncate text-sm mb-1" :title="playlist.title">{{ playlist.title }}</h3>
                <p class="text-xs text-gray-400 truncate mb-2">{{ playlist.channel }}</p>
                <div class="mt-auto flex items-center justify-between opacity-60 hover:opacity-100 transition-opacity pt-2">
                   <span class="text-[10px] uppercase tracking-wider font-semibold text-gray-500">Playlist</span>
                   <a :href="playlist.url" target="_blank" @click.stop class="text-gray-400 hover:text-white" title="Ouvrir sur YouTube">
          ...
          <!-- État Vide -->
          <div v-if="results.length === 0 && !isLoading && searchQuery" class="text-center py-20 text-gray-500">
...
// Suggestions
const suggestions = [
  { id: '1', title: 'Top Hits 2024', icon: '🔥', color: 'from-orange-500 to-red-600', query: 'Top Hits Global 2024' },
  { id: '2', title: 'Nouveautés', icon: '✨', color: 'from-purple-500 to-indigo-600', query: 'New Releases Music 2024' },
  { id: '3', title: 'Hits Été', icon: '☀️', color: 'from-yellow-400 to-orange-500', query: 'Summer Hits 2024' },
  { id: '4', title: 'Ambiance Soirée', icon: '🎉', color: 'from-pink-500 to-purple-600', query: 'Party Dance Mix 2024' },
  { id: '5', title: 'Lo-Fi Chill', icon: '☕', color: 'from-teal-400 to-blue-500', query: 'Chill Lo-fi Beats' },
  { id: '6', title: 'Travail Focus', icon: '💻', color: 'from-blue-600 to-cyan-500', query: 'Deep Focus Music' },
  { id: '7', title: 'Jazz Relax', icon: '🎷', color: 'from-amber-600 to-orange-800', query: 'Relaxing Jazz Masterpieces' },
  { id: '8', title: 'Rock Classics', icon: '🎸', color: 'from-red-700 to-black', query: 'Best Rock Songs of all time' },
]
